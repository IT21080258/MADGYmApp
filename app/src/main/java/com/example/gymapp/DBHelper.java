...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase gymDB) {
        ...
        gymDB.execSQL("create Table subscription(CardNumber TEXT primary key ,month TEXT, year TEXT)");
    }

    @Override
    public void onUpgrade(SQLiteDatabase gymDB, int i, int i1) {
        gymDB.execSQL("drop table if exists User");
        gymDB.execSQL("drop table if exists subscription");
    }
    ...
    public boolean addSubData(String cardNo, String exMonth, String exYear){
        SQLiteDatabase gymDB = getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("CardNumber", cardNo);
        contentValues.put("month", exMonth);
        contentValues.put("year", exYear);
        long result = gymDB.insert("subscription",null, contentValues);
        if (result == -1)
            return false;
        else
            return true;
